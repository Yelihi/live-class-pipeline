...
3. **[리뷰어]** 보완 및 최종 사양 확정 + 구현 후 실제 동작 검증 확인
4. **[테스터]** 검증 시나리오 및 smoke test 실행 (비즈니스 로직이 있을 때만)
5. **[참고 문헌 작성자]** 활용 문서 정리

`--pr` 플래그가 붙은 경우: 새 브랜치(`feat/t{번호}-{설명}`)를 생성하고 PR을 오픈한다. 없으면 main에 직접 커밋·push한다.
...
- 파이프라인 스크립트 파일명: `{번호}-{설명}.sh` 컨벤션 준수
